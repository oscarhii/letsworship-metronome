document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyRoleUi(){
    ...
    document.querySelectorAll('#beat-dots button,#song-sections button,#worship-cues button,#cue-grid button,.setlist-manager .song-actions button,.setlist-manager .row-main').forEach(node=>{node.classList.toggle('host-locked',follower);node.setAttribute('aria-disabled',String(follower));});
  }
  ...
  document.querySelectorAll('.segment-btn').forEach((button) => {
    button.onclick = () => { const parts=button.textContent.trim().split('/');sync.sendTimeSignature(parseInt(parts[0],10),parseInt(parts[1],10)||4);updateUI(); };
  });
  ...
  el('sync-setlist').onclick=()=>showCue(uiLanguage==='zh'?'歌單已同步給所有樂手':'Setlist synced to all musicians');
  ...
  sync.onAppEvent=payload=>{if(!payload)return;if(payload.type==='SONG_COUNTDOWN')showSongSwitchCountdown(payload.index,payload.count);if(payload.type==='SONG'){clearSongSwitchCountdown();selectSong(payload.index,true);}if(payload.type==='SECTION'){selectSong(payload.index,true);currentSection=Math.max(0,payload.section||0);renderWorshipFull();}if(payload.type==='CUE')showCue(payload.text,payload.tone,payload.subtitle,true);};
